...
class Pkb(models.Model):
    patterns = models.TextField(max_length=500, verbose_name="Patterns", blank=True)
    strategies = models.TextField(max_length=100, verbose_name="Strategia", blank=True)
    description = models.TextField(max_length=500, verbose_name="Descrizione", blank=True)
    context = models.TextField(max_length=500, verbose_name="Contesto", blank=True)
    mvc = models.TextField(max_length=100, verbose_name="Collocazione MVC", blank=True)

class Iso(models.Model):
    pkb = models.ForeignKey(Pkb, on_delete=models.CASCADE)
    iso = models.TextField(max_length=100, verbose_name="Iso", blank=True)
...
class Owasp(models.Model):
    ...
    top_ten = models.CharField(choices=TopTen.choices, blank=True, max_length=3)
    pkb = models.ForeignKey(Pkb, on_delete=models.CASCADE, blank=True)


class Example(models.Model):
    example = models.TextField(max_length=500, verbose_name="Exemple", blank=True)
    pkb = models.ForeignKey(Pkb, on_delete=models.CASCADE, blank=True)
...
class PrivacyByDesign(models.Model):
    ...
    design = models.CharField(choices=Design.choices, blank=True, max_length=3)
    pkb = models.ForeignKey(Pkb, on_delete=models.CASCADE)
